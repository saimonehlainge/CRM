...
{
    [Authorize]
    public class PrefixModel : PageModel
    {
        private readonly IUnitOfWork _unitOfWork;

        public PrefixModel(IUnitOfWork unitOfWork)
        { 
        ...
        public async Task<IActionResult> OnPostTable()
        {
            try
            {
                ...
                var GetDB = await _unitOfWork.PrefixRepositoy.GetAllAsync();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> OnPostAddData(RequestDTO.PrefixRequest request)
        {
            var i = 0;
            try
            {
                ...
                prefix.UpdatedDate = DateTime.Now;
                await _unitOfWork.PrefixRepositoy.AddAsync(prefix);
        ...
        [HttpGet]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> OnGetPrefix(int? Id)
        {
            var db = await _unitOfWork.PrefixRepositoy.GetByIdAsync(Id);
            return new JsonResult(db);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> OnPostEditData(RequestDTO.PrefixRequest request)
        {
            var i = 0;
            try
            {
                var prefix = await _unitOfWork.PrefixRepositoy.GetByIdAsync(request.Id);
        ...
        public async Task<IActionResult> OnGetDeleteData(int? Id)
        {
            ...
            try
            {
                var prefix = await _unitOfWork.PrefixRepositoy.GetByIdAsync(Id);
                if (prefix is not null)
                {
                    prefix.DeleteAt = 1;
                    await _unitOfWork.CompleteAsync();
