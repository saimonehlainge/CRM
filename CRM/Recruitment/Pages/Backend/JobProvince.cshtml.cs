...
{
    [Authorize]
    public class JobProvinceModel : PageModel
    {
        private readonly IUnitOfWork _unitOfWork;

        public JobProvinceModel(IUnitOfWork unitOfWork)
        {
        ...
        public async Task<IActionResult> OnPostTable()
        {
            try
            {
                ...
                var GetDB = await _unitOfWork.ProvincesRepository.GetAllProvinces();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> OnPostAddData(RequestDTO.ProvincesRequest request)
        {
            var i = 0;
            try
            {
                ...
                provinces.NameInThai = request.NameInThai;
                provinces.NameInEnglish = request.NameInEnglish;
                provinces.Status = request.Status;
                provinces.CreatedDate = DateTime.Now;
                provinces.UpdatedDate = DateTime.Now;
                await _unitOfWork.ProvincesRepository.AddAsync(provinces);
        ...
        [HttpGet]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> OnGetProvinces(int? Id)
        {
            var db = await _unitOfWork.ProvincesRepository.GetByIdAsync(Id);
            return new JsonResult(db);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> OnPostEditData(RequestDTO.ProvincesRequest request)
        {
            var i = 0;
            try
            {
                var provinces = await _unitOfWork.ProvincesRepository.GetByIdAsync(request.Id);
        ...
        public async Task<IActionResult> OnGetDeleteData(int? Id)
        {
            ...
            try
            {
                var marriedStatus = await _unitOfWork.ProvincesRepository.GetByIdAsync(Id);
