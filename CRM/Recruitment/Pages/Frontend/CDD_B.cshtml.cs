...
{
    public class CDD_BModel : PageModel
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly ILoggerHelperRepository _logger;

        public CDD_BModel(IUnitOfWork unitOfWork, ILoggerHelperRepository logger)
        {
        ...
        public async Task<IActionResult> OnPostFormB(RequestDTO.DTORequest request)
        {
            try
            { 
        ...
        public async Task<IActionResult> OnGetPosition()
        {
            ...
            db = db.Where(x => x.Status == 1 && x.TypeForm == 2 && x.DeleteAt != 1).ToList();
            return new JsonResult(db);
        }
