...
{
    public class CDD_CModel : PageModel
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly ILoggerHelperRepository _logger;

        public CDD_CModel(IUnitOfWork unitOfWork, ILoggerHelperRepository logger)
        {
        ...
        public async Task<IActionResult> OnPostFormC(RequestDTO.DTORequest request)
        {
            try
            { 
        ...
        public async Task<IActionResult> OnGetPosition()
        {
            ...
            db = db.Where(x => x.Status == 1 && x.TypeForm == 3 && x.DeleteAt != 1).ToList();
            return new JsonResult(db);
        }
